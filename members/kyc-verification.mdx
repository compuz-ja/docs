...
You need the **members.kyc.submit** capability to submit verification requests. The Credit Officer and the Tenant Administrator hold this capability, so either can run an automated check or record a manual attestation.
...
- **Escalate to senior reviewer**: this closes the review as Escalated and records the chosen senior reviewer (Credit Manager or Tenant Administrator) plus a reason code in the audit log. The member's KYC status does not change; they stay in **MANUAL_REVIEW**, but the review task itself is now closed.
