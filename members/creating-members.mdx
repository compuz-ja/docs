...
You need the **members.create** capability to add new member profiles. This capability is held by the Credit Officer and the Tenant Administrator. If you cannot see the **New Member** button, ask your Tenant Administrator to check your role assignment.
...
Deleting a member requires the **members.delete** capability, which is restricted to the Tenant Administrator. When a member is deleted, they are hidden from the standard Members list and search results. Their record is not permanently erased. It is preserved in full for regulatory audit purposes and can be restored by a Tenant Administrator if needed.
