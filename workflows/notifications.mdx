...
Emails to borrowers are off by default. They are controlled at the deployment level by the `ILOAN_MEMBER_EMAILS_ENABLED` environment setting, not by a screen the Tenant Administrator manages. Until that setting equals `true`, member recipients are filtered out before any email is enqueued. When it is switched on, members receive emails for two events only:
...
<Tip>
If you are not getting notifications you expect, check two things. First, confirm with your Tenant Administrator that your role is configured with the right capabilities. Second, check whether the loan is assigned to someone else. If it is, that person is the only one receiving stage notifications for that file.
</Tip>
