...
Each fee line carries a `taxable` flag. GCT applies only to lines flagged taxable. Financial-service and insurance-service fees are GCT-exempt, so a processing fee is not taxable. Legal and valuation fees are taxable and carry GCT.

The tenant default GCT rate is `gctRateBps`, which defaults to `1500`, that is 15%, the Jamaica standard. It is stored on the tenant configuration.

A **Disbursement** officer can override the GCT rate for a single disbursement. The override is passed per schedule and clamped to the range 0 to 10000 basis points. When omitted, the tenant default applies. This covers a special rate or a zero-rated case without changing the tenant default.

## The two fee tables

Fees live in two places: an editable per-tenant catalogue, and an immutable snapshot on each instruction.

| Table | What it is |
| --- | --- |
| `TenantFeeScheduleItem` | The editable per-tenant fee/tax catalogue. Officers read these as defaults per loan type, and a tenant edits them. |
...
A tenant created through the demo seed gets ten Jamaica fee items spread across the loan types. The defaults are starting points; a tenant edits them.
