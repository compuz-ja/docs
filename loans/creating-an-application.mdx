...
- **You have the Credit Officer role.** You need permission to create and submit loan applications. Contact your Tenant Administrator if you are unsure about your role.
...
<Steps>
  ...
  <Step title="Select the loan type">
    Choose the loan type. The dropdown lists the loan types your credit union has defined, and the visible names are configurable per tenant. Each one maps to one of four fixed archetypes that drive the behaviour: **Unsecured**, **Cash secured**, **Motor vehicle** (AUTO), or **Real estate** (MORTGAGE). The loan type determines which form sections appear. See [Loan Types](/loans/loan-types) for a full breakdown.
