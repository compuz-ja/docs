---
title: "Configure your credit union workspace settings"
...
The tenant setup area is where a Tenant Administrator controls your workspace branding, branches, and the settings fixed at onboarding.

Each credit union on the platform has its own isolated workspace called a **tenant**. Your workspace is identified by a unique slug that appears in every URL your team uses. As a **Tenant Administrator** you can configure branding and view your branches. A few settings are fixed at onboarding and cannot be changed afterward.

**Required role:** Tenant Administrator (`admin.tenant.manage` capability)

## Your tenant slug

Your tenant slug is the short, URL-safe identifier assigned to your credit union at onboarding. It appears in every URL your team uses, for example:
...
The slug appears in every URL your team uses, and it is baked into your onboarding records. Loan IDs do not use the slug: they embed the **branch code** instead, for example `MDVL-AUTO-00001`. Audit records are keyed to your tenant, not to the slug text. Because it is fixed into so many permanent links, **the slug cannot be changed after onboarding**.

<Warning>
  Your tenant slug cannot be changed after onboarding. Every URL references this
...
The first Tenant Administrator account is created during onboarding using the email address provided in the setup configuration. Once you are signed in with that account, you can invite additional administrators and staff from **Administration → Invitations**.
